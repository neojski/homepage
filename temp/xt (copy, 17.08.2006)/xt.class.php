...
class xt{
	...
	public function __destruct(){
		...
		//echo '<pre style="border: 1px solid">'.htmlspecialchars($this->xml->savexml()).'</pre>';
	}
	...
	public function getNode($name){
		if(is_node($name)){
			...
			$this->error(is_string($name)?'Obiekt o id "'.$name.'" nie istnieje':'Obiekt domnode nie istnieje');
			return null;
		}
	}
	/**
	...
	/**
	 * pobiera pierwszy element wg podanej klasy
	 */
	public function getElementByClassName($node, $class){
		if($t=$this->getElementsByClassName($node, $class)){
			return $t[0];
		}else{
			return null;
		}
	 ...
	 */
	private function getElementById($node, $id){
		foreach($node->getElementsByTagName('*') as $tag){
